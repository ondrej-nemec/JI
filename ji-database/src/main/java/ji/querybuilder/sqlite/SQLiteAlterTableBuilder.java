...
public class SQLiteAlterTableBuilder extends QueryBuilderParent implements AlterTableBuilder {
	
	private boolean first = true;

	public SQLiteAlterTableBuilder(Connection connection, String name) {
		super(connection);
	...
	public AlterTableBuilder addColumn(String name, ColumnType type, Object defaultValue, ColumnSetting... settings) {
		...
		query.append(EnumToSQLiteString.typeToString(type));
		query.append(EnumToSQLiteString.defaultValueToString(defaultValue));
		StringBuilder append = new StringBuilder();
		for (ColumnSetting setting : settings) {
			query.append(EnumToSQLiteString.settingToString(setting, name, append));
		}
	...
	public AlterTableBuilder addForeingKey(String column, String referedTable, String referedColumn, OnAction onDelete, OnAction onUpdate) {
		...
		query.append(String.format(
				...
				EnumToSQLiteString.onActionToString(onDelete),
				EnumToSQLiteString.onActionToString(onUpdate)
		));
		return this;
	}
	...
	public AlterTableBuilder modifyColumnType(String name, ColumnType type) {
		first();
		query.append(String.format("MODIFY %s %s", name, EnumToSQLiteString.typeToString(type)));
		return this;
	}
	...
	public AlterTableBuilder renameColumn(String originName, String newName, ColumnType type) {
		first();
		query.append(String.format("CHANGE COLUMN %s %s %s", originName, newName, EnumToSQLiteString.typeToString(type)));
		return this;
	}
